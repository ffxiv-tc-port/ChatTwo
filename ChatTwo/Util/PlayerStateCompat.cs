...
/// <summary>
/// TC note: TC's bundled Dalamud has no <c>Dalamud.Plugin.Services.IPlayerState</c> at all (a
/// newer Dalamud addition - see the shared skill notes' "recurring old-API-generation compile
/// fixes" list for the general "IPlayerState doesn't exist -> use IClientState" rule). This
/// wraps <see cref="Dalamud.Plugin.Services.IClientState"/>/its <c>LocalPlayer</c> to expose the
/// same shape (<c>IsLoaded</c>/<c>ContentId</c>/<c>CharacterName</c>/<c>HomeWorld</c>) the rest of
